#ifndef LAWN_MOWING_DISPERSION_SPARSIFICATION_H
#define LAWN_MOWING_DISPERSION_SPARSIFICATION_H

#include "mowing/witnesses/sparsification/WitnessSparsification.h"
#include "mowing/utils/mowing.h"

namespace mowing {

    class DispersionSparsification : public WitnessSparsification {
    public:
        std::optional<PointVector> sparsify(PointVector &witnesses,
                                            std::size_t n,
                                            const std::optional<ConicPolygon> &region,
                                            const std::optional<InputPolygon> &inputPolygon) override;

    };
}

#endif
